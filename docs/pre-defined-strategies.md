...
| Strategy Type | Associated Enum Value | Description |
|---------------|-----------------------|-------------|
| `NoDocumentationExtractionStrategy` | `DocumentationExtractionStrategyKind.`<br>`None` |  Documentation extraction strategy that does not extract documentation. |
| `XmlFileDocumentationExtractionStrategy` | `DocumentationExtractionStrategyKind.`<br>`FromXmlFiles` | Documentation extraction strategy that extract documentation from xml documentation files that are generated by the compiler. For more information about this strategy see [Customization Guide](customization.md). |

### Method Signature Extraction Strategies

| Strategy Type | Associated Enum Value | Description |
|---------------|-----------------------|-------------|
| `DefaultMethodSignatureExtractionStrategy` | `MethodSignatureExtractionStrategyKind.`<br>`Default` |  Method signature extraction strategy that ignore method parameters that have an ignore attribute, and replaces types that do not make any sense in rpc like the Task type. |
| `ResembleProtoClientMethodsStrategy` | `MethodSignatureExtractionStrategyKind.`<br>`ResembleProtoClientMethod` | Extract method signature so it will resemble proto client method. i.e. if and rpc has a request type then the grpc client will have a method with the following parameters: request type, headers, deadline, cancellationToken. For more information see [ResembleProtoClientMethodStrategySample](../src/SampleApp/Samples/StrategiesSamples/ResembleProtoClientMethodStrategySample.cs) |